...
Write-Host $newline
Write-Host "################ Start MASTER BUILDER  ################" -ForegroundColor cyan
Write-Host $newline
...
Add-Content -Path $master_file_path -Value "BEGIN TRY`n    BEGIN TRANSACTION"
...
Add-Content -Path $master_file_path -Value "    COMMIT`r`n `nEND TRY`nBEGIN CATCH`n    ROLLBACK`nEND CATCH"
